...
static void ioapic_write_internal(uintptr_t base, uint32_t reg, uint32_t value) {
    if (!base) {
        serial_printf(COM1, "IOAPIC: Attempt to write to unmapped IOAPIC (reg: 0x%x)\n", reg);
        return;
    }
...
static uint32_t ioapic_read_internal(uintptr_t base, uint32_t reg) {
    if (!base) {
        serial_printf(COM1, "IOAPIC: Attempt to read from unmapped IOAPIC (reg: 0x%x)\n", reg);
        return 0;
    }
...
void ioapic_redirect_irq(uint8_t irq, uint8_t vector, uint32_t flags) {
    ...
    if (irq >= max_redirects) {
        serial_printf(COM1, "IOAPIC: IRQ %u out of range (max %u)\n", irq, max_redirects);
        return;
    }
    ...
    serial_printf(COM1, "IOAPIC: IRQ %u redirected to vector 0x%x\n", irq, vector);
}
