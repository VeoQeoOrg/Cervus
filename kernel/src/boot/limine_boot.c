...
{
    if (hhdm) g_bi.hhdm_offset = hhdm->offset;

    if (fb && fb->framebuffer_count > 0) {
        struct limine_framebuffer *f = fb->framebuffers[0];
    ...
    if (rsdp && rsdp->address) g_bi.rsdp_addr = (uint64_t)(uintptr_t)rsdp->address;
    ...
    if (mods) {
        int n = 0;
        for (uint64_t i = 0; i < mods->module_count && n < BOOT_MOD_MAX; i++) {
            struct limine_file *mf = mods->modules[i];
            g_bi.modules[n].addr = (uint64_t)(uintptr_t)mf->address;
            g_bi.modules[n].size = mf->size;
            g_bi.modules[n].path = mf->path;
            n++;
        }
        g_bi.module_count = n;
    }
