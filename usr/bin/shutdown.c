...
static int elevate(const char *action)
{
    ...
    if (r != 0) {
        if (r == -1 || r == -13)
            fputs("shutdown: authentication failure\n", stderr);
        else
            fputs("shutdown: not permitted (you are not a sudoer)\n", stderr);
        return -1;
    }
    return 0;
}
...
static const char USAGE[] =
    "Usage: shutdown\nPower off the system. Requires root or the root password.\n";
int main(int argc, char **argv)
{
    ...
    if (!confirm_prompt()) {
        fputs(C_GREEN "\nShutdown cancelled." C_RESET "\n", stdout);
        return 0;
    }
    putchar('\n');

    if (elevate("shutdown") != 0)
        return 1;
