...
for stub in libm libdl libpthread librt libc; do
...
printf 'build initramfs.tar: initramfs bin/kernel usr/apps/init.elf%s %s builder/VERSION builder/mk_initramfs.sh | %s %s %s\n\n' \
    "$ALL_ELFS" "$SYSROOT_DATA" "$LIBCERVUS_A" "$SYSLIB/libm.a $SYSLIB/libdl.a $SYSLIB/libpthread.a $SYSLIB/librt.a $SYSLIB/libc.a" "$LIMINE_STAMP"
