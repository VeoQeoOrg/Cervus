...
USAGE="Usage: builder/hwtest.sh [--kvm|--tcg] [--timeout S] [pattern]
Boot the ISO across a matrix of emulated machines and report which
configurations reach userspace, how fast, and which fault.
A pattern is a shell glob restricting the run to matching profiles.
Results and per-machine serial logs land in hwtest-out/."
...
while [ $# -gt 0 ]; do
  case "$1" in
    --tcg) USAGE="Usage: builder/hwtest.sh [--kvm|--tcg] [--timeout S] [pattern]
Boot the ISO across a matrix of emulated machines and report which
configurations reach userspace, how fast, and which fault.
A pattern is a shell glob restricting the run to matching profiles.
Results and per-machine serial logs land in hwtest-out/."
...
ACCEL=tcg ;;
    ...
    -*) echo "hwtest: unknown option $1" >&2; echo "$USAGE" >&2; exit 2 ;;
    *) PATTERN=$1 ;;
  esac
  shift
done
