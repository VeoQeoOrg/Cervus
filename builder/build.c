...
static void path_join2(char *dst, size_t dst_size, const char *a,
                       const char *b) {
  if (dst_size == 0)
    return;
  size_t la = strlen(a);
...
const char *DIRS_TO_CLEAN[] = {"bin",
                               "obj",
                               "iso_root",
                               "limine",
                               "kernel/linker-scripts",
                               "demo_iso",
                               "limine-tools",
                               "edk2-ovmf",
                               INITRAMFS_ROOTFS,
                               NULL};
const char *FILES_TO_CLEAN[] = {"Cervus.iso",
                                "Cervus.hdd",
...
const char *SSE_FILES[] = {"sse.c",      "fpu.c",    "fabs.c",
                           "pow.c",      "pow10.c",  "serial.c",
                           "snprintf.c", "printf.c", NULL};
...
struct Dependency DEPENDENCIES[] = {
    {"freestnd-c-hdrs", "https://codeberg.org/OSDev/freestnd-c-hdrs-0bsd.git",
     "5df91dd7062ad0c54f5ffd86193bb9f008677631"},
    {"cc-runtime", "https://codeberg.org/OSDev/cc-runtime.git",
     "dae79833b57a01b9fd3e359ee31def69f5ae899b"},
    {"limine-protocol", "https://codeberg.org/Limine/limine-protocol.git",
...
static bool elf_up_to_date(const char *src, const char *elf) {
  ...
  if (get_mtime(src) > et)
    return false;
  if (get_mtime(SYSROOT_LIB "/libcervus.a") > et)
    return false;
  if (get_mtime(SYSROOT_LIB "/crt0.o") > et)
    return false;
  return true;
}
...
bool setup_dependencies(void) {
  ...
  for (int i = 0; DEPENDENCIES[i].name != NULL; i++) {
    ...
    if (!file_exists(path)) {
      print_color(COLOR_YELLOW, "Missing %s, setting up...",
                  DEPENDENCIES[i].name);
      if (cmd_run(true, "git clone %s %s", DEPENDENCIES[i].url, path) != 0)
        return false;
      char gc[PATH_MAX + 128];
      snprintf(gc, sizeof(gc),
               "git -C %s -c advice.detachedHead=false checkout %s", path,
               DEPENDENCIES[i].commit);
      if (system(gc) != 0)
        return false;
    }
  }
...
#define LIMINE_TARBALL_URL                                                     \
  ...
  "v" LIMINE_VERSION "/" LIMINE_TARBALL
...
bool build_limine(void) {
  ...
  print_color(COLOR_GREEN, "Fetching Limine v%s binary release...",
              LIMINE_VERSION);
  if (file_exists("limine"))
    rm_rf("limine");
  rm_rf(LIMINE_TARBALL);

  if (cmd_run(true, "curl -fL --retry 3 -o %s %s", LIMINE_TARBALL,
              LIMINE_TARBALL_URL) != 0) {
  ...
  if (!file_exists("limine-binary/Makefile")) {
    print_color(COLOR_RED,
                "Limine tarball did not contain limine-binary/Makefile");
    return false;
  }
  if (cmd_run(true, "mv limine-binary limine") != 0)
    return false;

  if (cmd_run(true, "make -C limine") != 0)
    return false;
  if (!extract_limine_hdd_bin())
    return false;
  return true;
}
...
static int scan_apps(void) {
  ...
  while ((de = readdir(d)) != NULL && g_naps < MAX_APPS) {
    ...
    if (nm[0] == '.')
      continue;
    if (strcmp(nm + nlen - 2, ".c") != 0)
      continue;

    app_entry_t *e = &g_apps[g_naps];
    snprintf(e->src, sizeof(e->src), "%s/%s", APPS_DIR, nm);
    snprintf(e->elf, sizeof(e->elf), "%s/%.*s.elf", APPS_DIR, (int)(nlen - 2),
             nm);
...
static bool build_one_app(const app_entry_t *e) {
  ...
  int ret =
      cmd_run(false,
              ...
              " -mno-red-zone"
              " -O0 -g"
              " -nostdinc -isystem " SYSROOT_INC " -Wl,-Ttext-segment=0x401000"
...
static int scan_bin_apps(void) {
  ...
  if (!d) {
    print_color(COLOR_YELLOW, "[bin] No '%s' directory found, skipping",
                BIN_APPS_DIR);
    return 0;
  }
  struct dirent *de;
  while ((de = readdir(d)) != NULL && g_nbin < MAX_APPS) {
    ...
    snprintf(e->elf, sizeof(e->elf), "%s/%.*s.elf", BIN_APPS_DIR,
             (int)(nlen - 2), nm);
    snprintf(e->name, sizeof(e->name), "%.*s", (int)(nlen - 2), nm);
    g_nbin++;
  }
  ...
  for (int i = 1; i < g_nbin; i++) {
    app_entry_t tmp = g_bin_apps[i];
    int j = i - 1;
    while (j >= 0 && strcmp(g_bin_apps[j].name, tmp.name) > 0) {
      g_bin_apps[j + 1] = g_bin_apps[j];
      j--;
    }
    g_bin_apps[j + 1] = tmp;
  }
...
bool build_all_bin_apps(void) {
  ...
  print_color(COLOR_CYAN, "[bin] Found %d program(s) in '%s'", g_nbin,
              BIN_APPS_DIR);
  for (int i = 0; i < g_nbin; i++) {
    if (!build_one_bin_app(&g_bin_apps[i]))
      return false;
  }
  return true;
}
...
static void dstr_init(dstr_t *s) {
  s->data = NULL;
  s->len = 0;
  s->cap = 0;
}
static void dstr_free(dstr_t *s) {
  free(s->data);
  s->data = NULL;
  s->len = s->cap = 0;
}
...
static bool scan_libcervus_sources(const char *dir, const char *rel,
                                   strvec_t *out) {
  char full[BIGPATH];
  if (rel && rel[0])
  ...
  while ((de = readdir(d)) != NULL) {
    ...
    if (rel && rel[0])
      snprintf(child_rel, sizeof(child_rel), "%s/%s", rel, nm);
    else
      snprintf(child_rel, sizeof(child_rel), "%s", nm);
...
bool build_libcervus(void) {
  ...
  print_color(COLOR_CYAN,
              "[libcervus] Building libcervus.a and crt0.o (native)...");
  ...
  (void)!chdir(cwd_buf);
  print_color(COLOR_GREEN, "[libcervus] OK (compiled %d, skipped %d)", built,
              skipped);
  return true;
}
...
static bool read_file_dstr(const char *path, dstr_t *out) {
  ...
  if (!dstr_reserve(out, (size_t)sz + 1)) {
    fclose(f);
    return false;
  }
...
static bool dstr_replace_once(dstr_t *s, const char *old_s, const char *new_s) {
  ...
  if (new_n > old_n) {
    if (!dstr_reserve(s, s->len + (new_n - old_n) + 1))
      return false;
    p = s->data + off;
  }
...
static bool dstr_guard_line_with(dstr_t *s, const char *marker,
  ...
  if (!dstr_reserve(s, s->len + ins_pre + ins_suf + 1))
    return false;
...
static int dstr_guard_func_impls(dstr_t *s, const char *name,
                                 const char *guard_macro) {
  int count = 0;
  size_t cursor = 0;
  ...
  while (cursor + name_n < s->len) {
    ...
    if (memmem(s->data + look, line_start - look, guard_macro,
               strlen(guard_macro))) {
      cursor = end;
      continue;
    }
    ...
    if (!dstr_reserve(s, s->len + pre_n + suf_n + 1))
      return count;

    memmove(s->data + end + pre_n + suf_n, s->data + end, s->len - end + 1);
    memcpy(s->data + end + pre_n, suffix, suf_n);
    memmove(s->data + line_start + pre_n, s->data + line_start,
            end - line_start);
...
static bool patch_file(const char *src_dir, const char *rel,
  ...
  if (changed > 0) {
    ...
    print_color(COLOR_YELLOW, "[tcc-patch] %s: no changes (already patched?)",
                rel);
  }
  dstr_free(&s);
  return true;
...
static int edit_tcc_h(dstr_t *s) {
  ...
  if (dstr_guard_line_with(s, "enable bound checking code",
                           "CONFIG_TCC_BCHECK"))
    changed++;
  return changed;
}
...
static int edit_libtcc_c(dstr_t *s) {
  ...
  if (dstr_replace_once(s,
                        ...
                        "(void)filename;\n    return NULL;\n"))
    changed++;
  if (dstr_replace_once(s, "return dlopen(filename, RTLD_LOCAL | RTLD_LAZY);\n",
                        "(void)filename;\n    return NULL;\n"))
    changed++;
  if (dstr_replace_once(s, "return dlsym(handle, sym);\n",
                        "(void)handle; (void)sym;\n    return NULL;\n"))
    changed++;
  if (dstr_replace_once(s, "dlclose(handle);\n", "(void)handle;\n"))
    changed++;

  if (!memmem(s->data, s->len, "s->alacarte_link = 1;\n    s->static_link = 1;",
              45)) {
    if (dstr_replace_once(s, "    s->alacarte_link = 1;",
                          "    s->alacarte_link = 1;\n    s->static_link = 1;"))
      changed++;
  }
...
static int edit_tccrun_c(dstr_t *s) {
  ...
  if (dstr_guard_line_with(s, "<sys/ucontext.h>", "TCC_NO_BACKTRACE"))
    changed++;
  ...
  if (!memmem(s->data, s->len, "typedef void *ucontext_t;",
    ...
    if (last_inc_end > 0) {
      ...
      if (dstr_reserve(s, s->len + stub_n + 1)) {
        memmove(s->data + last_inc_end + stub_n, s->data + last_inc_end,
  ...
  if (dstr_guard_line_with(s, "set_exception_handler();", "TCC_NO_BACKTRACE"))
    changed++;

  const char *names[] = {"rt_error", "sig_error", "set_exception_handler",
                         "rt_get_caller_pc", NULL};
...
static int edit_tccelf_c(dstr_t *s) {
  ...
  if (!memmem(s->data, s->len, "tcc_add_library_err(s1, \"cervus\")", 33)) {
    if (dstr_replace_once(
            s,
            "    if (!s1->nostdlib) {\n        tcc_add_library_err(s1, \"c\");",
            "    if (!s1->nostdlib) {\n        tcc_add_library_err(s1, "
            "\"cervus\");"))
      changed++;
  }
  ...
  if (!memmem(s->data, s->len, "CERVUS_PLT_FOLD", 15)) {
    if (dstr_replace_once(
            ...
            "                rel->r_info = ELFW(R_INFO)(sym_index, "
            "R_X86_64_PC32);\n"
            "                continue;\n"
            "            }",
...
static bool tcc_extract_and_patch(void) {
  ...
  if (!file_exists(src_dir_full)) {
    ...
    if (cmd_run(true, "tar -xjf %s/%s -C %s", TCC_DIR, TCC_TAR_FNAME,
                TCC_DIR) != 0) {
  ...
  if (!patch_file(src_dir_full, "tcc.h", edit_tcc_h))
    return false;
  if (!patch_file(src_dir_full, "libtcc.c", edit_libtcc_c))
    return false;
  if (!patch_file(src_dir_full, "tccrun.c", edit_tccrun_c))
    return false;
  if (!patch_file(src_dir_full, "tccelf.c", edit_tccelf_c))
    return false;
  return true;
}
...
static bool tcc_build_and_install(void) {
  ...
  char one_x86_64_link[SHORTPATH], one_x86_64_gen[SHORTPATH],
      one_tccpp[SHORTPATH];
  ...
  path_join2(one_x86_64_link, sizeof(one_x86_64_link), src_dir_full,
             "x86_64-link.c");
  path_join2(one_x86_64_gen, sizeof(one_x86_64_gen), src_dir_full,
             "x86_64-gen.c");
  path_join2(one_tccpp, sizeof(one_tccpp), src_dir_full, "tccpp.c");

  long elf_t = get_mtime_safe(tcc_elf);
  bool need_tcc = !file_exists(tcc_elf) || get_mtime_safe(one_src) > elf_t ||
  ...
  if (need_tcc) {
    ...
    if (!file_exists(crt0)) {
      print_color(COLOR_RED, "[tcc] %s missing (libcervus build failed?)",
                  crt0);
      return false;
    }
    int rc =
  ...
  if (need_libtcc1) {
    ...
    const char *l1_flags = "-ffreestanding -nostdlib -fno-stack-protector "
                           "-mno-red-zone -fno-pie -fno-pic -O2 -D__CERVUS__ "
                           "-DTCC_TARGET_X86_64";
    ...
    char ar_cmd[2048];
    size_t arpos =
        (size_t)snprintf(ar_cmd, sizeof(ar_cmd), "ar rcs %s", libtcc1_a);
    ...
    if (cmd_run(true, "gcc %s -c %s -o %s", l1_flags, src_path, l1_o) != 0) {
      print_color(COLOR_RED, "[tcc] libtcc1.c compile failed");
      return false;
    }
    arpos +=
        (size_t)snprintf(ar_cmd + arpos, sizeof(ar_cmd) - arpos, " %s", l1_o);

    path_join2(src_path, sizeof(src_path), src_dir_full, "lib/alloca86_64.S");
    if (file_exists(src_path)) {
      if (cmd_run(true, "gcc %s -c %s -o %s", l1_flags, src_path, al_o) != 0) {
        print_color(COLOR_RED, "[tcc] alloca86_64.S compile failed");
        return false;
      }
      arpos +=
          (size_t)snprintf(ar_cmd + arpos, sizeof(ar_cmd) - arpos, " %s", al_o);
    }

    path_join2(src_path, sizeof(src_path), src_dir_full, "lib/va_list.c");
    if (file_exists(src_path)) {
      if (cmd_run(true, "gcc %s -c %s -o %s", l1_flags, src_path, va_o) != 0) {
        print_color(COLOR_RED, "[tcc] va_list.c compile failed");
        return false;
      }
      arpos +=
          (size_t)snprintf(ar_cmd + arpos, sizeof(ar_cmd) - arpos, " %s", va_o);
    }

    if (cmd_run(true, "%s", ar_cmd) != 0) {
      print_color(COLOR_RED, "[tcc] ar failed");
      return false;
    }
  } else {
  ...
  if (cmd_run(false, "mkdir -p %s/usr/bin %s/usr/lib/tcc/include", SYSROOT_DIR,
              SYSROOT_DIR) != 0) {
    print_color(COLOR_RED, "[tcc] mkdir failed");
    return false;
  }
  if (cmd_run(false, "cp %s %s/usr/bin/tcc", tcc_elf, SYSROOT_DIR) != 0 ||
      cmd_run(false, "cp %s %s/usr/lib/tcc/libtcc1.a", libtcc1_a,
              SYSROOT_DIR) != 0 ||
      cmd_run(false, "cp %s/include/*.h %s/usr/lib/tcc/include/", src_dir_full,
              SYSROOT_DIR) != 0) {
...
bool build_tcc(void) {
  if (!tcc_download())
    return false;
  if (!tcc_extract_and_patch())
    return false;
  if (!tcc_build_and_install())
    return false;
  return true;
}
...
void clean_tcc_build(bool deep) {
  ...
  cmd_run(false, "rm -f %s/libtcc1.o %s/alloca86_64.o %s/va_list.o", TCC_DIR,
          TCC_DIR, TCC_DIR);
  if (file_exists(tcc_elf)) {
    remove(tcc_elf);
    print_color(COLOR_YELLOW, "[clean] removed %s", tcc_elf);
  }
  if (file_exists(libtcc1_a)) {
    remove(libtcc1_a);
    print_color(COLOR_YELLOW, "[clean] removed %s", libtcc1_a);
  }

  cmd_run(false, "rm -f %s/usr/bin/tcc", SYSROOT_DIR);
  cmd_run(false, "rm -rf %s/usr/lib/tcc", SYSROOT_DIR);
  print_color(COLOR_YELLOW,
              "[clean] removed %s/usr/bin/tcc and %s/usr/lib/tcc/", SYSROOT_DIR,
              SYSROOT_DIR);
  ...
  if (deep) {
    ...
    if (file_exists(src_dir)) {
      rm_rf(src_dir);
      print_color(COLOR_YELLOW, "[clean] removed %s", src_dir);
    }
    if (file_exists(tar_path)) {
      remove(tar_path);
      print_color(COLOR_YELLOW, "[clean] removed %s", tar_path);
    }
  }
}
...
bool build_installer(void) {
  ...
  if (!d) {
    print_color(COLOR_YELLOW, "[installer] directory '%s' not found, skipping",
                INSTALLER_DIR);
    return true;
  }
  struct dirent *de;
  ...
  while ((de = readdir(d)) != NULL) {
    ...
    snprintf(e.elf, sizeof(e.elf), "%s/%.*s.elf", INSTALLER_DIR,
             (int)(nlen - 2), nm);
    snprintf(e.name, sizeof(e.name), "%.*s", (int)(nlen - 2), nm);
    if (!build_one_app(&e)) {
      closedir(d);
  ...
  closedir(d);
  if (built == 0)
    print_color(COLOR_YELLOW, "[installer] no .c sources in '%s'",
                INSTALLER_DIR);
  return true;
}
...
bool build_initramfs(void) {
  ...
  if (!build_limine()) {
    print_color(COLOR_YELLOW, "[initramfs] warning: limine not available yet, "
                              "boot files will be incomplete");
  }
  ...
  const char *dirs[] = {INITRAMFS_ROOTFS "/bin",
  ...
  if (file_exists(INIT_ELF)) {
    ...
    print_color(COLOR_RED,
                "[initramfs] init.elf not found - boot will drop to nothing!");
  ...
  scan_bin_apps();
  print_color(COLOR_CYAN,
              "[initramfs] Copying %d bin program(s) -> rootfs/bin/", g_nbin);
  for (int i = 0; i < g_nbin; i++) {
    if (!file_exists(g_bin_apps[i].elf)) {
      print_color(COLOR_YELLOW, "[initramfs] %s not built, skipping",
                  g_bin_apps[i].src);
      continue;
    }
    char dst[512];
    snprintf(dst, sizeof(dst), "%s/bin/%s", INITRAMFS_ROOTFS,
             g_bin_apps[i].name);
    if (cmd_run(false, "cp %s %s", g_bin_apps[i].elf, dst) != 0) {
      print_color(COLOR_RED, "[initramfs] Failed to copy %s",
                  g_bin_apps[i].elf);
    } else {
  ...
  scan_apps();
  for (int i = 0; i < g_naps; i++) {
    if (strcmp(g_apps[i].name, "init") == 0)
      continue;
    if (!file_exists(g_apps[i].elf))
      continue;
    char dst[512];
  ...
  if (readme) {
    fprintf(readme, "Cervus OS v0.0.2\n"
                    "================\n"
                    "\n"
  ...
  struct {
    ...
    bool required;
      ...
      {"limine/limine-bios.sys", INITRAMFS_ROOTFS "/boot/limine-bios.sys",
       false},
      {"limine/limine-bios-hdd.bin",
       INITRAMFS_ROOTFS "/boot/limine-bios-hdd.bin", false},
  ...
  for (int i = 0; boot_items[i].src; i++) {
    if (!file_exists(boot_items[i].src)) {
      if (boot_items[i].required) {
        print_color(COLOR_RED, "[initramfs] missing required boot file: %s",
                    boot_items[i].src);
        return false;
      }
      print_color(COLOR_YELLOW, "[initramfs] skip (not built yet): %s",
                  boot_items[i].src);
      continue;
    }
    if (cmd_run(false, "cp %s %s", boot_items[i].src, boot_items[i].dst) != 0) {
      ...
      print_color(COLOR_GREEN, "[initramfs] %s -> %s", boot_items[i].src,
                  boot_items[i].dst);
    }
  }

  print_color(COLOR_CYAN, "[initramfs] Packing %s...", INITRAMFS_TAR);
  if (cmd_run(false, "tar --format=ustar -cf %s -C %s .", INITRAMFS_TAR,
              INITRAMFS_ROOTFS) != 0) {
...
bool is_sse_file(const char *filename) {
  for (int i = 0; SSE_FILES[i] != NULL; i++)
    if (strstr(filename, SSE_FILES[i]) != NULL)
      return true;
  return false;
}
...
bool compile_kernel(void) {
  ...
  FileList sources = {0};
  if (file_exists("kernel/src"))
    find_src_files("kernel/src", &sources);
  if (file_exists("libc/src"))
    find_src_files("libc/src", &sources);
  ...
  for (int i = 0; i < sources.count; i++) {
    ...
    if (strncmp(src, "kernel/", 7) == 0)
      strcpy(category, "kernel");
    else if (strncmp(src, "libc/", 5) == 0)
      strcpy(category, "libc");
    ...
    if (file_exists(obj_path) && get_mtime(src) <= get_mtime(obj_path) &&
        get_mtime("builder/build.c") <= get_mtime(obj_path))
      continue;
    ...
    if (strcmp(ext, ".psf") == 0) {
      ...
      *strrchr(stem, '.') = '\0';
      ...
      if (cmd_run(false, "nasm -g -F dwarf -f elf64 %s -o %s", src, obj_path) !=
          0)
        failed = true;
      ...
      if (cmd_run(false, "gcc %s %s -c %s -o %s", flags, cppflags, src,
                  obj_path) != 0)
        failed = true;
    }
  }
...
bool create_iso(void) {
  ...
  if (file_exists(WALLPAPER_SRC)) {
    ...
    print_color(COLOR_YELLOW, "Warning: wallpaper not found at %s",
                WALLPAPER_SRC);
  }
  ...
  if (has_initramfs) {
    ...
    print_color(COLOR_GREEN,
                "[module 1] initramfs.tar -> iso_root/boot/initramfs.tar");
  } else {
    print_color(COLOR_YELLOW,
                "[module 1] initramfs.tar not found - booting without rootfs");
  }

  FILE *f = fopen("limine.conf", "w");
  if (!f) {
    print_color(COLOR_RED, "Cannot create limine.conf");
    return false;
  }
  ...
  if (has_elf) {
    fprintf(f, "    module_path: boot():/boot/shell.elf\n"
               "    module_cmdline: init\n");
  }
  if (has_initramfs) {
    fprintf(f, "    module_path: boot():/boot/initramfs.tar\n"
  ...
  snprintf(iso_name, sizeof(iso_name), "demo_iso/%s.%s.%s.iso", IMAGE_NAME,
           VERSION, timestamp);
...
bool build_data_iso(void) {
  ...
  if (f) {
    fprintf(f, "Cervus OS sample data CD\n"
               "------------------------\n"
  ...
  if (f) {
    fprintf(f, "This file lives in a nested directory (docs/).\n"
...
void flash_iso(void) {
  ...
  list_iso_files(&isos);
  if (isos.count == 0) {
    print_color(COLOR_RED, "No ISO found in demo_iso/");
    return;
  }
  ...
  int ic;
  printf("Choice: ");
  if (scanf("%d", &ic) < 1 || ic < 1 || ic > isos.count)
    return;

  list_usb_devices(&devs);
  if (devs.count == 0) {
    print_color(COLOR_RED, "No removable USB devices found!");
    return;
  }
  ...
  for (int i = 0; i < devs.count; i++) {
    ...
    snprintf(mpath, sizeof(mpath), "/sys/block/%s/device/model",
             devs.paths[i] + 5);
  ...
  int dc;
  printf("Choice: ");
  if (scanf("%d", &dc) < 1 || dc < 1 || dc > devs.count)
    return;

  printf("\n%sWARNING: ALL DATA ON %s WILL BE ERASED!%s\n", COLOR_BOLD,
         devs.paths[dc - 1], COLOR_RESET);
  ...
  print_color(COLOR_YELLOW, "Flashing %s -> %s ...", isos.paths[ic - 1],
              devs.paths[dc - 1]);
...
bool is_unreadable_file(const char *filename) {
  ...
  for (int i = 0; skip_names[i]; i++)
    if (strcmp(filename, skip_names[i]) == 0)
      return true;
  ...
  const char *bin_exts[] = {
      ".psf", ".jpg", ".png", ".jpeg", ".iso",       ".hdd",  ".img", ".bin",
      ".elf", ".o",   ".txt", ".md",   ".gitignore", ".json", ".tar", NULL};
...
void generate_tree_recursive(const char *base_dir, FILE *out, int level) {
  ...
  const char *skip_dirs[] = {"wallpapers", "demo_iso",       ".vscode",
                             "builder",    INITRAMFS_ROOTFS, NULL};
...
static bool __attribute__((unused)) copy_file_raw(const char *src,
                                                  const char *dst) {
  FILE *in = fopen(src, "r");
  if (!in)
    return false;
...
static void write_cervus_entries(FILE *f, bool has_elf, bool has_initramfs) {
  ...
  fprintf(f,
          ...
          IMAGE_NAME, VERSION);
  if (has_elf) {
    fprintf(f, "    module_path: boot():/cervus/shell.elf\n"
               "    module_cmdline: init\n");
  }
  if (has_initramfs) {
    fprintf(f, "    module_path: boot():/cervus/initramfs.tar\n"
               "    module_cmdline: initramfs\n");
  }
...
void hardware_test(void) {
  ...
  if (!file_exists(LIMINE_CONF_PATH)) {
    ...
    print_color(
        COLOR_YELLOW,
        "Install Limine first: https://github.com/limine-bootloader/limine");
    return;
  }
  ...
  if (already_has_cervus) {
    ...
    if (!file_exists(LIMINE_CONF_BACKUP)) {
      ...
      if (cmd_run(true, "cp %s %s", LIMINE_CONF_PATH, LIMINE_CONF_BACKUP) !=
          0) {
      ...
      print_color(COLOR_YELLOW,
                  "To restore later: ./build --resethardwareconf");
    } else {
  ...
  if (has_initramfs) {
    ...
    print_color(COLOR_GREEN, "  initramfs.tar -> %s/initramfs.tar",
                CERVUS_BOOT_DIR);
  }

  if (file_exists(WALLPAPER_SRC)) {
    cmd_run(true, "cp %s %s/wallpaper.png", WALLPAPER_SRC, CERVUS_BOOT_DIR);
    print_color(COLOR_GREEN, "  wallpaper -> %s/wallpaper.png",
                CERVUS_BOOT_DIR);
  }
  ...
  if (!already_has_cervus) {
    ...
    print_color(COLOR_CYAN,
                "Step 4: Updating Cervus entries in limine.conf...");
    ...
    if (!tmp) {
      fclose(orig);
      print_color(COLOR_RED, "Cannot create temp file");
      return;
    }
  ...
  print_color(COLOR_YELLOW, "First boot:    select '%s %s (Install / Live)'",
              IMAGE_NAME, VERSION);
  print_color(COLOR_YELLOW,
              "After install: select '%s %s (Installed - boot from disk)'",
              IMAGE_NAME, VERSION);
  print_color(COLOR_YELLOW,
              "To restore original config: sudo ./build --resethardwareconf");
...
static const char *find_ovmf(void) {
  const char *ovmf_paths[] = {"/usr/share/edk2/x64/OVMF.4m.fd",
...
static void alex_test(void) {
  ...
  print_color(
      COLOR_CYAN,
      "Building with AddressSanitizer (host) and checking for leaks...");
  ...
  if (!build_tcc()) {
    print_color(COLOR_RED, "tcc build failed");
    return;
  }
  if (!build_all_apps()) {
    print_color(COLOR_RED, "apps build failed");
    return;
  }
  if (!build_all_bin_apps()) {
    print_color(COLOR_RED, "bin apps build failed");
    return;
  }
  if (!build_installer()) {
    print_color(COLOR_RED, "installer build failed");
    return;
  }
  if (!compile_kernel()) {
    print_color(COLOR_RED, "kernel build failed");
    return;
  }
  if (!build_initramfs()) {
    print_color(COLOR_RED, "initramfs build failed");
...
static void do_clean(void) {
  for (int i = 0; DIRS_TO_CLEAN[i]; i++)
    rm_rf(DIRS_TO_CLEAN[i]);
  clean_apps_elfs();
  clean_bin_elfs();
  for (int i = 0; FILES_TO_CLEAN[i]; i++)
    if (file_exists(FILES_TO_CLEAN[i]))
      remove(FILES_TO_CLEAN[i]);
  clean_libcervus_build(true);
  clean_tcc_build(true);
...
static void print_help(void) {
  ...
  printf("  ./build run [options]    Build kernel + initramfs + ISO, launch "
         "QEMU\n");
  printf(
      "  ./build flash            Flash latest ISO to a USB device (sudo)\n");
  printf("  ./build hardware         Install Cervus into the Limine boot menu "
         "(sudo)\n");
  ...
  printf("  --installed      Boot existing disk only, no ISO (simulate real "
         "HW)\n");
...
static void prepare_disks(const run_cfg_t *c) {
  if (c->disk == DISK_NONE)
    return;
  if (c->disk == DISK_ALL) {
    const char *imgs[] = {"cervus_ata.img", "cervus_sata.img",
                          "cervus_nvme.img", NULL};
    for (int i = 0; imgs[i]; i++) {
      ...
      if (!file_exists(imgs[i])) {
        ...
        cmd_run(false, "dd if=/dev/zero of=%s bs=1M count=256 2>/dev/null",
                imgs[i]);
      }
    }
    if (!file_exists("cervus_data.iso"))
      build_data_iso();
    return;
  }
  ...
  if (!file_exists("cervus_disk.img")) {
    ...
    cmd_run(false,
            "dd if=/dev/zero of=cervus_disk.img bs=1M count=256 2>/dev/null");
  } else {
...
static void launch_qemu(const run_cfg_t *c, const char *iso) {
  ...
  if (c->uefi) {
    ...
    if (!ovmf) {
      print_color(COLOR_RED, "OVMF not found (sudo apt install ovmf)");
      return;
    }
  ...
  if (c->disk == DISK_ALL) {
    ...
    cmd_run(false,
            ...
            " -drive "
            "id=sata0,file=cervus_sata.img,format=raw,if=none,file.locking=off"
            " -device ide-hd,bus=ahci.0,drive=sata0,bootindex=3"
            " -drive "
            "id=cd_data,file=cervus_data.iso,format=raw,if=none,media=cdrom,"
            "file.locking=off"
            " -device ide-cd,bus=ahci.1,drive=cd_data"
            " -drive "
            "id=nvm0,file=cervus_nvme.img,format=raw,if=none,file.locking=off"
  ...
  if (c->disk == DISK_AHCI) {
    ...
    cmd_run(
        ...
        " -drive "
        "id=hd0,file=cervus_disk.img,format=raw,if=none,file.locking=off"
  ...
  if (c->disk == DISK_NVME) {
    ...
    cmd_run(
        ...
        "GDK_BACKEND=x11 qemu-system-x86_64 -machine q35%s -cdrom %s -boot d"
        " -drive "
        "id=nvm0,file=cervus_disk.img,format=raw,if=none,file.locking=off"
...
static int launch_installed(const run_cfg_t *c) {
  if (!file_exists("cervus_disk.img")) {
    print_color(COLOR_RED,
                "cervus_disk.img not found. Run './build run' first.");
    return 1;
  }
  char bios[PATH_MAX] = "";
  if (c->uefi) {
  ...
  if (c->disk == DISK_NVME) {
    cmd_run(false,
            ...
            " -drive "
            "id=nvm0,file=cervus_disk.img,format=raw,if=none,file.locking=off"
            " -device nvme,serial=CRV001,drive=nvm0 -serial stdio %s 2>&1 | "
            "tee log.txt",
            bios, QEMUFLAGS);
  } else {
    cmd_run(false,
            "GDK_BACKEND=x11 qemu-system-x86_64 -machine pc%s"
            " -drive file=cervus_disk.img,format=raw,if=ide -serial stdio %s "
            "2>&1 | tee log.txt",
            bios, QEMUFLAGS);
  }
  return 0;
...
static void tui_render(const run_cfg_t *c, int sel) {
  ...
  const char *items[MENU_ROWS] = {"Firmware",
                                  "Disk",
                                  "Fresh disk",
                                  "Boot installed",
                                  "Keep artifacts",
  ...
  for (int i = 0; i < MENU_ROWS; i++) {
    ...
    if (val[i][0])
      printf("%s%-26s %s%s%s\n", cur, items[i], COLOR_YELLOW, val[i],
             COLOR_RESET);
    else
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(a, "--tree") == 0) {
      ...
      if (strcmp(d, "ide") == 0)
        cfg.disk = DISK_IDE;
      else if (strcmp(d, "ahci") == 0)
        cfg.disk = DISK_AHCI;
      else if (strcmp(d, "nvme") == 0)
        cfg.disk = DISK_NVME;
      else if (strcmp(d, "all") == 0)
        cfg.disk = DISK_ALL;
      else if (strcmp(d, "none") == 0)
        cfg.disk = DISK_NONE;
      else {
        print_color(COLOR_RED, "Unknown --disk=%s", d);
        return 1;
      }
    } else if (a[0] != '-') {
      command = a;
  ...
  if (strcmp(command, "alex") == 0 || ARG_ALEX) {
    alex_test();
    return 0;
  }
