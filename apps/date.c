...
static int cmos_read(int reg) {
    int w = ioport_write(0x70, 1, (uint32_t)(reg & 0x7F));
    if (w < 0) return -1;
    int v = (int)ioport_read(0x71, 1);
    if (v < 0) return -1;
    return v & 0xFF;
}
...
static void cmos_wait_ready(void) {
    for (int i = 0; i < 2000; i++) {
        ioport_write(0x70, 1, 0x0A);
        int sta = (int)ioport_read(0x71, 1);
        if (sta < 0 || !(sta & 0x80)) return;
    }
}
...
static int64_t rtc_read_unix(void) {
    ...
    ioport_write(0x70, 1, 0x0B);
    int regb        = (int)ioport_read(0x71, 1);
    ...
    for (int m = 1; m < mon; m++)
        days += MDAYS[m-1] + (m == 2 && is_leap(year) ? 1 : 0);
    days += mday - 1;
