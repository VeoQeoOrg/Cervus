...
static inline int meminfo(cervus_meminfo_t *m) { return (int)syscall1(SYS_MEMINFO, m); }
...
static inline int usleep(unsigned int usecs) {
    nanosleep_simple((uint64_t)usecs * 1000ULL);
    return 0;
}
...
static inline size_t strspn(const char *s, const char *accept) {
    ...
    while (s[n]) {
        int found = 0;
        for (size_t i = 0; accept[i]; i++) if (s[n] == accept[i]) { found = 1; break; }
        if (!found) break;
        n++;
    }
    return n;
...
static inline size_t strcspn(const char *s, const char *reject) {
    size_t n = 0;
    while (s[n]) {
        for (size_t i = 0; reject[i]; i++) if (s[n] == reject[i]) return n;
        n++;
    }
    return n;
}
...
static inline int isprint(int c)  { return (unsigned char)c >= 0x20 && (unsigned char)c < 0x7F; }
static inline int isgraph(int c)  { return (unsigned char)c > 0x20 && (unsigned char)c < 0x7F; }
...
static inline int     atoi(const char *s)   { return (int)atoi64(s); }
static inline long    atol(const char *s)   { return (long)atoi64(s); }
static inline int64_t atoll(const char *s)  { return atoi64(s); }

static inline int64_t strtol(const char *s, char **end, int base) {
    while (isspace((unsigned char)*s)) s++;
    int neg = 0;
    if (*s == '-') { neg = 1; s++; } else if (*s == '+') s++;
    if ((base == 0 || base == 16) && s[0] == '0' && (s[1] == 'x' || s[1] == 'X')) { s += 2; base = 16; }
    else if (base == 0 && *s == '0') { s++; base = 8; }
...
static inline uint64_t strtoul(const char *s, char **end, int base) {
    while (isspace((unsigned char)*s)) s++;
    if (*s == '+') s++;
    if ((base == 0 || base == 16) && s[0] == '0' && (s[1] == 'x' || s[1] == 'X')) { s += 2; base = 16; }
    else if (base == 0 && *s == '0') { s++; base = 8; }
    ...
    while (1) {
        ...
        if (d >= base) break;
        v = v * (uint64_t)base + (uint64_t)d; s++;
    }
    if (end) *end = (char *)s;
    return v;
}
...
static inline int64_t llabs(int64_t x) { return x < 0 ? -x : x; }

static inline double fabs(double x) { return x < 0.0 ? -x : x; }

static inline double __cervus_pow_int(double base, int64_t e) {
...
static inline double pow(double base, double exp) {
    int64_t ie = (int64_t)exp;
    if ((double)ie == exp) return __cervus_pow_int(base, ie);
    if (base <= 0.0) return 0.0;
    double x = base; int n = 0;
    while (x > 1.5) { x /= 2.0; n++; }
    while (x < 0.75) { x *= 2.0; n--; }
    double y = (x - 1.0) / (x + 1.0), y2 = y * y, term = y, lnx = 0.0;
    for (int i = 0; i < 20; i++) { lnx += term / (2 * i + 1); term *= y2; }
    lnx = lnx * 2.0 + (double)n * 0.6931471805599453;
    double t = exp * lnx, et = 1.0, term2 = 1.0;
    for (int i = 1; i < 30; i++) { term2 *= t / (double)i; et += term2; if (fabs(term2) < 1e-15) break; }
    return et;
}
static inline double sqrt(double x) {
    ...
    for (int i = 0; i < 60; i++) { double nr = (r + x / r) * 0.5; if (fabs(nr - r) < 1e-15 * r) { r = nr; break; } r = nr; }
    return r;
}
...
static inline void ws(const char *s)  { write(1, s, strlen(s)); }
static inline void wc(char c)         { write(1, &c, 1); }
static inline void wn(void)           { write(1, "\n", 1); }
static inline void wse(const char *s) { write(2, s, strlen(s)); }

static inline void print_u64(uint64_t v)  { char b[22]; u64_to_dec(v, b); ws(b); }
static inline void print_i64(int64_t v)   { if (v < 0) { wc('-'); print_u64((uint64_t)(-v)); } else print_u64((uint64_t)v); }
static inline void print_hex(uint64_t v)  { char b[20]; u64_to_hex(v, b); ws(b); }
static inline void print_pad(uint64_t v, int width) {
    char t[22]; int i = 21; t[i] = '\0';
    if (!v) t[--i] = '0'; else while (v) { t[--i] = '0' + v % 10; v /= 10; }
    int len = 21 - i;
    for (int p = len; p < width; p++) wc(' ');
    ws(t + i);
}
...
static inline void printf_fd(int fd, const char *fmt, ...) {
    ...
    int zero_pad = 0;
    while (*fmt) {
        if (*fmt != '%') { write(fd, fmt, 1); fmt++; width = 0; zero_pad = 0; continue; }
        fmt++;
        zero_pad = (*fmt == '0');
        width = 0;
        ...
        switch (*fmt) {
            case 's': {
                ...
                if (width > slen) for (int p = slen; p < width; p++) write(fd, " ", 1);
                write(fd, s, slen);
                break;
            }
            case 'd': case 'i': {
                ...
                int l = 0; while (nb[l]) l++;
                if (width > l) for (int p = l; p < width; p++) write(fd, zero_pad ? "0" : " ", 1);
            ...
            case 'z': {
                fmt++;
                if (*fmt == 'u') {
                    size_t v = __builtin_va_arg(ap, size_t);
                    u64_to_dec((uint64_t)v, nb);
                    write(fd, nb, strlen(nb));
                }
                break;
...
static inline int snprintf(char *buf, size_t sz, const char *fmt, ...) {
    ...
    size_t pos = 0;
    char nb[32];
#define _SN_PUT(s, n) do { size_t _n = (n); for (size_t _i = 0; _i < _n && pos + 1 < sz; _i++) buf[pos++] = (s)[_i]; } while(0)
    while (*fmt && pos + 1 < sz) {
        ...
        switch (*fmt) {
            ...
            case 'd': case 'i': {
                int64_t v = is_long ? __builtin_va_arg(ap, int64_t) : (int64_t)__builtin_va_arg(ap, int);
                if (v < 0) { buf[pos++] = '-'; v = -v; }
                u64_to_dec((uint64_t)v, nb); _SN_PUT(nb, strlen(nb)); break;
            }
            case 'u': {
                uint64_t v = is_long ? __builtin_va_arg(ap, uint64_t) : (uint64_t)__builtin_va_arg(ap, unsigned);
                u64_to_dec(v, nb); _SN_PUT(nb, strlen(nb)); break;
            }
            case 'x': case 'X': {
                uint64_t v = is_long ? __builtin_va_arg(ap, uint64_t) : (uint64_t)__builtin_va_arg(ap, unsigned);
                u64_to_hex(v, nb); _SN_PUT(nb + 2, strlen(nb + 2)); break;
            }
            case 'p': {
                uint64_t v = (uint64_t)__builtin_va_arg(ap, void *);
                u64_to_hex(v, nb); _SN_PUT(nb, strlen(nb)); break;
            }
            case 'c': { char c = (char)__builtin_va_arg(ap, int); if (pos + 1 < sz) buf[pos++] = c; break; }
            case '%': if (pos + 1 < sz) buf[pos++] = '%'; break;
            default:  if (pos + 1 < sz) buf[pos++] = '%'; if (pos + 1 < sz) buf[pos++] = *fmt; break;
        }
        fmt++;
    }
#undef _SN_PUT
...
#define fputs(s, fd)         write((fd), (s), strlen(s))
#define putchar(c)           wc((char)(c))
#define getchar()            ({ char _c; read(0, &_c, 1) > 0 ? (int)(unsigned char)_c : -1; })
...
static inline void path_join(const char *base, const char *name, char *out, size_t sz) {
    if (!name || !name[0]) { strncpy(out, base, sz - 1); out[sz - 1] = '\0'; return; }
    if (name[0] == '/') { strncpy(out, name, sz - 1); out[sz - 1] = '\0'; return; }
    strncpy(out, base, sz - 1); out[sz - 1] = '\0';
...
static inline void path_norm(char *path) {
    char tmp[512]; strncpy(tmp, path, 511); tmp[511] = '\0';
...
static inline void resolve_path(const char *cwd, const char *path, char *out, size_t sz) {
    if (!path || !path[0]) { strncpy(out, cwd, sz - 1); out[sz - 1] = '\0'; return; }
    if (path[0] == '/') { strncpy(out, path, sz - 1); out[sz - 1] = '\0'; return; }
...
static inline int is_shell_flag(const char *a) {
    if (!a) return 0;
    if (a[0]=='-' && a[1]=='-' && a[2]=='c' && a[3]=='w' && a[4]=='d' && a[5]=='=') return 1;
    if (a[0]=='-' && a[1]=='-' && a[2]=='e' && a[3]=='n' && a[4]=='v' && a[5]==':') return 1;
    return 0;
}

static inline const char *getenv_argv(int argc, char **argv, const char *name, const char *def) {
    for (int i = 1; i < argc; i++) {
        const char *a = argv[i];
