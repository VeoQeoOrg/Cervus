...
typedef struct { int64_t tv_sec; int64_t tv_nsec; } cervus_timespec_t;
...
static inline int strncmp(const char *a,const char *b,size_t n){
    for(size_t i=0;i<n;i++){
        if(a[i]!=b[i])return(uint8_t)a[i]-(uint8_t)b[i];if(!a[i])return 0;
    }
    return 0;
}
...
static inline char *strchr(const char *s,int c){
    for(;*s;s++)if(*s==(char)c)return(char*)s;return c==0?(char*)s:0;
}
...
static inline void print_pad(uint64_t v,int width){
    ...
    int len=21-i;for(int p=len;p<width;p++)wc(' ');ws(t+i);
}

static inline void print_pad0(uint64_t v,int width){
    char t[22];int i=21;t[i]='\0';
    if(!v)t[--i]='0';else while(v){t[--i]='0'+v%10;v/=10;}
    int len=21-i;for(int p=len;p<width;p++)wc('0');ws(t+i);
}
...
static inline void printf_fd(int fd,const char *fmt,...){
    ...
    while(*fmt){
        ...
        switch(*fmt){
            case 's':{const char *s=__builtin_va_arg(ap,const char*);if(!s)s="(null)";write(fd,s,strlen(s));break;}
            case 'd':{int64_t v=is_long?__builtin_va_arg(ap,int64_t):(int64_t)__builtin_va_arg(ap,int);
                if(v<0){write(fd,"-",1);v=-v;}u64_to_dec((uint64_t)v,nb);write(fd,nb,strlen(nb));break;}
            case 'u':{uint64_t v=is_long?__builtin_va_arg(ap,uint64_t):(uint64_t)__builtin_va_arg(ap,unsigned);
                u64_to_dec(v,nb);write(fd,nb,strlen(nb));break;}
            case 'x':case 'X':{uint64_t v=is_long?__builtin_va_arg(ap,uint64_t):(uint64_t)__builtin_va_arg(ap,unsigned);
                u64_to_hex(v,nb);write(fd,nb+2,strlen(nb+2));break;}
            case 'p':{uint64_t v=(uint64_t)__builtin_va_arg(ap,void*);u64_to_hex(v,nb);write(fd,nb,strlen(nb));break;}
            case 'c':{char c=(char)__builtin_va_arg(ap,int);write(fd,&c,1);break;}
...
static inline int readline(int fd,char *buf,int maxlen){
    int i=0;
    while(i<maxlen-1){char c;ssize_t r=read(fd,&c,1);
        if(r<=0){buf[i]='\0';return i>0?i:-1;}
        if(c=='\r')continue;buf[i++]=c;if(c=='\n')break;}
    buf[i]='\0';return i;
}
...
static inline void path_join(const char *base,const char *name,char *out,size_t sz){
    ...
    size_t nl=strlen(name);if(bl+nl+1<sz)memcpy(out+bl,name,nl+1);
}
...
static inline void path_norm(char *path){
    ...
    for(int i=0;i<np;i++){out[ol++]='/';size_t pl=strlen(parts[i]);memcpy(out+ol,parts[i],pl);ol+=pl;}
