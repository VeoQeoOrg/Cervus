...
static void cmd_help(void) {
    ...
    ws("  mount, umount, mkfs, lsblk, mv, rm, mkdir, touch\n");
...
static int launch_installer(void) {
    ...
    cervus_stat_t st;
    if (stat(path, &st) != 0) {
        ws(C_RED "  install-on-disk not found on system.\n" C_RESET);
        return -1;
    }
    ...
    pid_t child = fork();
    if (child < 0) {
        ws(C_RED "  fork failed\n" C_RESET);
        return -1;
    }
    if (child == 0) {
        execve(path, argv, NULL);
        ws(C_RED "  exec install-on-disk failed\n" C_RESET);
        exit(127);
    }
    int status = 0;
    waitpid(child, &status, 0);
...
static int ask_install_or_live(void) {
    ws("\x1b[2J\x1b[H");
    ws("\n");
    ws(C_CYAN "  Cervus OS" C_RESET " - Live ISO\n");
    ws(C_GRAY "  -----------------------------------" C_RESET "\n\n");
    ws("  A disk has been detected on this machine.\n");
    ws("  What would you like to do?\n\n");
    ws("    [" C_BOLD "1" C_RESET "] Install Cervus to disk\n");
    ws("    [" C_BOLD "2" C_RESET "] Continue in Live mode\n\n");
    ws("  Choice [1-2]: ");

    char c = 0;
    while (1) {
        if (read(0, &c, 1) <= 0) continue;
        if (c == '1' || c == '2') { wc(c); wn(); break; }
    }
    return (c == '1') ? 1 : 0;
}
