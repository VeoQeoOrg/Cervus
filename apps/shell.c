    ...
    for(;;){
        ...
        if(c=='\x1b'){
            ...
            if(s[1]=='A'){
                ...
                if(hidx<hist_count){hidx++;const char *h=hist_get(hidx);
                    if(h){vt_left(pos);vt_eol();int hl=slen(h);
                        if(hl>limit)hl=limit;
                        for(int i=0;i<hl;i++)buf[i]=h[i];
                        buf[hl]='\0';len=hl;pos=len;write(1,buf,len);}}
                continue;}
            if(s[1]=='B'){
                if(hidx>0){hidx--;const char *h=(hidx==0)?saved:hist_get(hidx);
                    if(!h)h="";vt_left(pos);vt_eol();int hl=slen(h);
                    if(hl>limit)hl=limit;
                    for(int i=0;i<hl;i++)buf[i]=h[i];
                    buf[hl]='\0';len=hl;pos=len;write(1,buf,len);}
                continue;}
            if(s[1]=='C'){if(pos<len){vt_right(1);pos++;}continue;}
            if(s[1]=='D'){if(pos>0){vt_left(1);pos--;}continue;}
            if(s[1]=='H'){if(pos>0){vt_left(pos);pos=0;}continue;}
            if(s[1]=='F'){if(pos<len){vt_right(len-pos);pos=len;}continue;}
            if(s[1]=='3'){char tilde;read(0,&tilde,1);
                if(pos<len){for(int i=pos;i<len-1;i++)buf[i]=buf[i+1];
                    len--;buf[len]='\0';write(1,buf+pos,len-pos);
                    vt_eol();if(len>pos)vt_left(len-pos);}
                continue;}
            continue;}
        if(c=='\n'||c=='\r'){buf[len]='\0';if(pos<len)vt_right(len-pos);wn();return len;}
        if(c==3){ws("^C");wn();buf[0]='\0';return 0;}
        if(c==4){if(len==0)return -1;continue;}
        if(c=='\b'||c==0x7F){
            if(pos>0){for(int i=pos-1;i<len-1;i++)buf[i]=buf[i+1];
                len--;pos--;buf[len]='\0';vt_left(1);
                write(1,buf+pos,len-pos);vt_eol();
...
static int tokenize(char *line,char *argv[],int maxargs){
    ...
    while(*p){
        ...
        else if(*p=='\''){p++;argv[argc++]=p;while(*p&&*p!='\'')p++;if(*p)*p++='\0';}
...
static int cmd_cd(const char *path){
    ...
    path_join_s(cwd,path,np,sizeof(np));path_norm_s(np);
    cervus_stat_t st;
...
static int run_single(char *line){
    ...
    if(cmd[0]=='/'||cmd[0]=='.'){
        ...
        int  cwd_in_apps = (cwd[0]=='/'&&cwd[1]=='a'&&cwd[2]=='p'&&
                            cwd[3]=='p'&&cwd[4]=='s'&&
                            (cwd[5]=='\0'||cwd[5]=='/'));
        path_join_s(cwd,cmd,t_cwd,sizeof(t_cwd));path_norm_s(t_cwd);
    ...
    real_argv[0] = binpath;
    if(seq(cmd,"pwd")||
       (slen(binpath)>=3&&seq(binpath+slen(binpath)-3,"pwd"))){
        if(argc<2){
            real_argv[1]=cwd;
            real_argv[2]=(void*)0;
            real_argc=2;
...
__attribute__((noreturn)) void _start_main(uint64_t *initial_rsp){
    ...
    for(;;){
        ...
        if(n<0){ws("\nSession ended. Restarting shell...\n");
            scpy(cwd,"/",sizeof(cwd));print_motd();continue;}
        int len=slen(line);
